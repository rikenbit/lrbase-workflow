...
GENEID_L <- unlist(lapply(ALIAS_L, function(x){
	tmp <- try(select(Homo.sapiens,
		c("GENEID"), keys=x, keytype="ALIAS"), silent=TRUE)
	if(class(tmp) == "try-error"){
		x
	}else{
		as.character(tmp[1, "GENEID"])
	}
}))
...
# GENEID_L
GENEID_L <- lapply(complex, function(x){
	...
	if(ALIAS_L == ""){
		...
		tmp <- try(select(Homo.sapiens,
			c("GENEID"), keys=ALIAS_L, keytype="ALIAS"), silent=TRUE)
		if(class(tmp) == "try-error"){
			ALIAS_L
		}else{
			as.character(tmp[, "GENEID"])
		}
	}else{
		tmp <- try(select(Homo.sapiens,
			c("GENEID"), keys=ALIAS_L, keytype="ALIAS"), silent=TRUE)
...
# GENEID_R
GENEID_R <- lapply(complex, function(x){
	...
	if(ALIAS_R == ""){
		...
		tmp <- try(select(Homo.sapiens,
			c("GENEID"), keys=ALIAS_R, keytype="ALIAS"), silent=TRUE)
		if(class(tmp) == "try-error"){
			ALIAS_R
		}else{
			as.character(tmp[, "GENEID"])
...
# PMID_PPI
Comments <- CP$comments_interaction[complex]
PMID_PPI <- unlist(lapply(Comments, function(x){
	m <- regexpr("[0-9][0-9][0-9][0-9][0-9][0-9][0-9]", x)
	substr(x, m, m + attr(m, "match.length") - 1)
}))
...
# L-R merge
cellphonedb_comp <- c()
for(x in 1:length(GENEID_L)){
	L <- GENEID_L[[x]]
	R <- GENEID_R[[x]]
	L <- L[which(!is.na(L))]
	R <- R[which(!is.na(R))]
	cellphonedb_comp <- rbind(cellphonedb_comp,
		cbind(expand.grid(L, R) , PMID_PPI[x]))
}
...
# cellphonedb
cellphonedb <- data.frame(cellphonedb, SOURCEDB="CellPhoneDB")
